...
eval list form = case form of
                Atom a -> fromJust (lookup a list)
                Not b -> not (eval list b)
                e1 :/\: e2 -> eval list e1 && eval list e2  
                e1 :\/: e2 -> eval list e1 || eval list e2  
...
evalTests =
    TestList [ "A /\\ (B \\/ ~B) with A = True" ~: eval [("A", True), ("B", True)] (Atom "A" :/\: (Atom "B" :\/: Not (Atom "B"))) ~?= True
             ...
             ," B \\/ (A /\\ ~A) with B = True" ~: eval[("A",True), ("B",True)] (Atom "A" :\/: ( Atom "B" :/\: Not ( Atom "B"))) ~?= True
             ," B \\/ (A /\\ ~A) with B = True" ~: eval[("A",False), ("B",True)] (Atom "A" :\/: ( Atom "B" :/\: Not ( Atom "B"))) ~?= False
             ]
...
nnf :: Formula -> Formula
nnf form = case form of
    Not (e1 :/\: e2 ) -> nnf (Not e1) :\/: nnf (Not e2)
    Not (e1 :\/: e2 ) -> nnf (Not e1) :/\: nnf (Not e2)
    (e1 :\/: e2 ) -> (nnf e1) :\/: (nnf e2)
    (e1 :/\: e2 ) -> (nnf e1) :/\: (nnf e2)
    Not (Not e1) -> nnf e1
...
nnfTests =
    TestList [ "double negation" ~: isNnf (nnf (Not (Not (Atom "x")))) ~?= True
             ...
             , "double de Morgans" ~: isNnf (nnf (Not ((Not (Atom "x")) :\/: (Not (Atom "y"))))) ~?= True
             , "double neg one side" ~: isNnf (nnf (Not((Not (Atom "x"))) :\/: (Not (Atom "y")))) ~?= True
             ]

--trace ("nnf with ddm" ++ show (Not (Not (Atom "x") :\/: ( Not (Atom "y"))))) (nnf (Not(Not (Atom "x") :\/: (Not(Atom "y")))))
...
formulae)
  ...
  Distribution of /\ over \/:
    A /\ (B \/ C) == (A /\ B) \/ (A /\ C)
    (A \/ B) /\ C == (A /\ C) \/ (B /\ C)
    -handle case for children 
    A /\ B  -> if ... then dnf $ dnf A /\ dnf B else dnf A /\ dnf B
...
dnf :: Formula -> Formula
dnf form = let innnf = nnf form
            in case innnf of
                (e0 :/\: e1) -> let exp = (dnf e0) :/\: (dnf e1) in 
                    case exp of
                        (e0 :/\:(e1 :\/: e2)) -> dnf exp
                        (e0 :/\:(e1 :/\: e2)) -> dnf exp
                        ((e0 :\/: e1) :/\: e2) -> dnf exp
...
dnfTests =
    TestList [ "right distribution" ~: isDnf (dnf (Atom "A" :/\: (Atom "B" :\/: Atom "C"))) ~?= True
             ...
             , "double left distribution" ~: isDnf (dnf (((Atom "A" :\/: Atom "B") :/\: Atom "C") :\/: Atom "D")) ~?= True
             , "double right distribution" ~: isDnf (dnf (Atom "D" :/\: (Atom "C" :/\: (Atom "A" :\/: Atom "B")))) ~?= True
             ]
